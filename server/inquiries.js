...
export async function adminInquiryAnswer({request,env,params}) {
  ...
  try {
    ...
    const content=object(input)?sanitizeStoredBoardContent(input.content):null;if(!object(input)||!content)return errorResponse('INVALID_ANSWER',400);
    ...
    const results=await env.DB.batch([
      env.DB.prepare('INSERT INTO inquiry_answers (id,inquiry_id,admin_user_id,content,created_at,updated_at) VALUES (?,?,?,?,?,?) ON CONFLICT(inquiry_id) DO UPDATE SET admin_user_id=excluded.admin_user_id,content=excluded.content,updated_at=excluded.updated_at').bind(crypto.randomUUID(),params.id,access.userId,content,now,now),

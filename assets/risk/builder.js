...
export function buildSurveyForm(form, survey = {}) {
  ...
  const base = { title: '', target: '', companyName: '', startDate: '', endDate: '', guidance: '현재 작업 중 느끼는 위험요인과 개선 의견을 작성해주세요.', departments: [],
    settings: { collectName: true, collectDepartment: true, collectEmployeeId: false, allowAnonymous: false, allowDuplicates: false, allowEdit: false, allowPhoto: true },
  ...
  for (const [key, label, suffix] of [['collectName','이름 수집','collect-name'],['collectDepartment','부서 수집','collect-department'],['collectEmployeeId','사번 수집','collect-employee-id'],['allowAnonymous','익명 응답 허용','allow-anonymous'],['allowDuplicates','중복 응답 허용 (사번 기준)','allow-duplicates'],['allowPhoto','사진 첨부 허용','allow-photo']]) {
  ...
  function renderQuestions() {
    ...
    questions.forEach((q, i) => {
      ...
      else { type.append(new Option('기본 위험성 문항 · ' + q.type, q.type)); type.disabled = true; }
      ...
      card.append(el('h3', `문항 ${i+1}`), title, type, requiredLabel);
      if (choiceType(q)) {
  ...
  questionSection.append(button('새 문항 추가',()=>{if(questions.length<30){questions.push({id:'c_'+crypto.randomUUID(),type:'single_choice',text:'새 질문',required:false,options:['선택지 1']});renderQuestions();}}),button('기본 템플릿 복원',()=>{if(confirm('현재 문항 구성을 기본 템플릿으로 바꾸시겠습니까?')){questions=structuredClone(DEFAULT_QUESTIONS);renderQuestions();}})); renderQuestions(); form.append(questionSection);
  return () => ({ schemaVersion: 2, revision: base.revision ?? 0, ...Object.fromEntries(Object.entries(fields).map(([key,input])=>[key,input.value.trim()])),
    settings: { ...base.settings, ...Object.fromEntries(Object.entries(checks).map(([key,input])=>[key,input.checked])) }, departments: departments.map(d=>d.trim()).filter(Boolean), questions: structuredClone(questions).map(q=>({...q,text:q.text.trim(),options:(q.options||[]).map(v=>v.trim())})), isActive: active.checked });
}

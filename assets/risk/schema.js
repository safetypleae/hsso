...
export const DEFAULT_QUESTIONS = [
  ...
  { id: 'q4', type: 'hazard_location', text: '위험 장소를 작성해주세요.' },
  ...
  { id: 'q8', type: 'photo', text: '현장 사진', required: false },
...
export function legacyQuestions(stored = []) {
  ...
  return DEFAULT_QUESTIONS.map(base => {
    ...
    return { ...base, text: old?.text || base.text, options: old?.options?.length ? old.options : base.options };
  });
}
...
export function visibleQuestion(q, questions, answers, settings) {
  if (q.type === 'photo' && !settings.allowPhoto) return false;
  if (!questions.some(v => v.id === 'q1')) return true;
  if (q.id === 'safe_reason') return answers.q1 === '아니오';
  if (/^q[2-8]$/.test(q.id)) return answers.q1 === '예';
  return true;
}

...
async function call(db, { cookie, body = { name: 'Changed' }, method = 'POST', requestOrigin = origin, raw, contentType = 'application/json', handler = profile } = {}) {
  const headers = {};
...
test('profile updates only the authenticated name and /me immediately reads the result', async t => {
  const { db, owner, other } = await fixture(t);
  const before = rows(db), sessions = db.sqlite.prepare('SELECT * FROM sessions').all();
  const result = await call(db, { cookie: owner.cookie, body: { name: '  수정 이름  ' } });
  ...
  assert.deepEqual({ ...saved, name: original.name, updated_at: original.updated_at }, { ...original });
  ...
  assert.equal((await call(db, { cookie: owner.cookie, handler: me, method: 'GET' })).data.user.name, '수정 이름');
});

for (const field of ['user_id', 'id', 'email', 'role', 'isAdmin', 'password', 'password_hash']) {
  test('profile rejects spoofed or read-only field: ' + field, async t => {
    const { db, owner, other } = await fixture(t), before = rows(db);
    const result = await call(db, { cookie: owner.cookie, body: { name: 'Changed', [field]: field === 'user_id' || field === 'id' ? other.id : 'attacker' } });
    assert.equal(result.status, 400); assert.deepEqual(rows(db), before);
...
test('profile validates name and accepts the existing 50-code-point limit', async t => {
  ...
  assert.deepEqual(rows(db), before);
  assert.equal((await call(db, { cookie: owner.cookie, body: { name: '😀'.repeat(50) } })).status, 200);
  const text = "<img src=x onerror=alert(1)>');--";
  assert.equal((await call(db, { cookie: owner.cookie, body: { name: text } })).data.user.name, text);

...
import {boardCollection,boardItem,MAX_BOARD_BODY_BYTES} from '../server/boards.js';
...
test('list separates board types and paginates',async t=>{const{db,a}=await fixture(t);await create(db,a);db.sqlite.prepare('INSERT INTO board_posts VALUES (?,?,?,?,?,?,?,?)').run(crypto.randomUUID(),'notice',null,'공지 제목','공지 본문',0,new Date().toISOString(),new Date().toISOString());assert.equal((await call(boardCollection,db,{path:'/api/boards?type=free'})).data.posts.length,1);assert.equal((await call(boardCollection,db,{path:'/api/boards?type=notice'})).data.posts.length,1);for(let i=0;i<22;i++)await create(db,a,{title:'글 '+i});const page=await call(boardCollection,db,{path:'/api/boards?type=free&limit=20&offset=0'});assert.equal(page.data.posts.length,20);assert.equal(page.data.hasMore,true);assert.equal(page.data.total,23);});
test('detail increments view count and missing post is 404',async t=>{const{db,a}=await fixture(t),post=await create(db,a);const one=await call(boardItem,db,{path:'/api/boards/'+post.id,params:{id:post.id}}),two=await call(boardItem,db,{path:'/api/boards/'+post.id,params:{id:post.id}});assert.equal(one.data.post.viewCount,1);assert.equal(two.data.post.viewCount,2);assert.equal((await call(boardItem,db,{params:{id:crypto.randomUUID()}})).status,404);});
test('free posting requires login, uses session owner, and notice creation stays closed',async t=>{const{db,a,b}=await fixture(t);assert.equal((await call(boardCollection,db,{method:'POST',data:payload()})).status,401);const post=await call(boardCollection,db,{cookie:a.cookie,method:'POST',data:payload({user_id:b.id,authorUserId:b.id})});assert.equal(post.status,201);assert.equal(db.sqlite.prepare('SELECT author_user_id FROM board_posts WHERE id=?').get(post.data.post.id).author_user_id,a.id);assert.equal((await call(boardCollection,db,{cookie:a.cookie,method:'POST',data:payload({boardType:'notice'})})).data.error,'ADMIN_REQUIRED');});
...
test('free posts sanitize and update rich source without changing ownership rules',async t=>{const{db,a,b}=await fixture(t),source=`${BOARD_RICH_MARKER}\n${JSON.stringify({version:1,blocks:[{type:'paragraph',align:'left',runs:[{text:'내용',bold:true,href:'javascript:alert(1)',onclick:'x'}]}]})}`,post=await create(db,a,{content:source}),path='/api/boards/'+post.id,params={id:post.id},stored=(await call(boardItem,db,{cookie:b.cookie,path,params})).data.post.content;assert.deepEqual(parseRichStorage(stored).blocks[0].runs,[{text:'내용',bold:true}]);const updated=`${BOARD_RICH_MARKER}\n${JSON.stringify({version:1,blocks:[{type:'ol',align:'left',items:[[{text:'확인'}]]}]})}`;assert.equal((await call(boardItem,db,{cookie:a.cookie,method:'PATCH',path,params,data:{title:'서식 수정',content:updated}})).status,200);assert.equal(parseRichStorage((await call(boardItem,db,{cookie:a.cookie,path,params})).data.post.content).blocks[0].type,'ol');});

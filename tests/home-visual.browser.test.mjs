...
test('home Scorpius background and narrative system visuals', {skip:!browserPath,timeout:45000}, async t=>{
  ...
  for(const width of [1920,1440,1024,768,390]){
    ...
    const home=await evaluate(`(()=>{const foreground=['.portal-brand','.portal-floating-notice','.portal-recent','.portal-quiz'].map(selector=>document.querySelector(selector).getBoundingClientRect());const first=document.querySelector('.hsso-constellation-stars circle'),last=document.querySelector('.hsso-constellation-stars circle:last-child');return {overflow:document.documentElement.scrollWidth<=innerWidth,foreground:foreground.every(rect=>rect.width>0&&rect.height>0),brand:document.querySelector('.portal-brand p').textContent.includes('Health & Safety Smart Office'),stars:document.querySelectorAll('.hsso-constellation-stars circle').length,lines:document.querySelectorAll('.hsso-constellation-lines path').length,antares:!!document.querySelector('.hsso-antares'),delays:[first.style.getPropertyValue('--d'),last.style.getPropertyValue('--d')],entrance:getComputedStyle(first).animationName.includes('hsso-constellation-enter'),network:!!document.querySelector('.hsso-safety-network'),shooting:[...document.querySelectorAll('.hsso-shooting-star')].every(star=>getComputedStyle(star).display!=='none'),rects:foreground.map(({x,y,width,height})=>[x,y,width,height])};})()`);
    assert.equal(home.overflow,true,`page 1 overflow ${width}`);assert.equal(home.foreground,true,`foreground visible ${width}`);assert.equal(home.brand,true,`HSSO full name ${width}`);assert.equal(home.stars,15,`star count ${width}`);assert.equal(home.lines,9,`line count ${width}`);assert.equal(home.antares,true,`Antares ${width}`);assert.deepEqual(home.delays,['.15s','3.4s'],`entrance delays ${width}`);assert.equal(home.entrance,true,`entrance animation ${width}`);assert.equal(home.network,false,`legacy network ${width}`);assert.equal(home.shooting,width>700,`shooting stars ${width}`);
    ...
    const systems=await evaluate(`(()=>{const grid=document.querySelector('.hsso-systems-grid'),cards=[...document.querySelectorAll('.hsso-system-card')],intro=document.querySelector('.hsso-introduction');return {columns:getComputedStyle(grid).gridTemplateColumns.split(' ').length,overflow:document.documentElement.scrollWidth<=innerWidth&&cards.every(card=>card.scrollWidth<=card.clientWidth)&&intro.scrollWidth<=intro.clientWidth,intro:intro.textContent.includes('Health & Safety Smart Office')&&intro.textContent.includes('안전보건 업무를 하나의 흐름으로'),chemical:!!document.querySelector('.chem-document')&&!!document.querySelector('.chem-analyzer')&&document.querySelectorAll('.chem-results span').length===5,risk:document.querySelectorAll('.risk-participants>span').length===3&&!!document.querySelector('.risk-survey-panel')&&!!document.querySelector('.risk-comment'),health:document.querySelectorAll('.health-task-list>div').length===4&&!!document.querySelector('.health-board'),animations:[getComputedStyle(document.querySelector('.chem-document')).animationName,getComputedStyle(document.querySelector('.risk-choice i'),'::after').animationName,getComputedStyle(document.querySelector('.health-next-task')).animationName]};})()`);
    assert.equal(systems.columns,width>=1001?3:width>=701?2:1,`system columns ${width}`);assert.equal(systems.overflow,true,`page 3 overflow ${width}`);assert.equal(systems.intro,true,`HSSO introduction ${width}`);assert.equal(systems.chemical,true);assert.equal(systems.risk,true);assert.equal(systems.health,true);assert.deepEqual(systems.animations,['chem-document-cycle','risk-option-cycle','health-next-task']);
  }

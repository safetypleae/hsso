...
test('chemical dashboard, product status and mobile layout', {skip:!browserPath,timeout:45000},async t=>{
  ...
  const project=fileURLToPath(new URL('..',import.meta.url));let base,manage=false,uploaded=false,productInfo={productName:'WD-40',manufacturer:'제조사',supplier:'공급사',productCode:'WD40',generalUse:'윤활',productStatus:'ACTIVE'};
  const server=createServer(async(req,res)=>{try{
    ...
    if(path==='/api/chemicals/dashboard')return respond(200,{ok:true,summary:{products:1,withMsds:uploaded?1:0,withoutMsds:uploaded?0:1,unreviewed:uploaded?1:0},departments:[{id:'22222222-2222-4222-8222-222222222222',name:'시설팀',products:1,withoutMsds:uploaded?0:1,unreviewed:uploaded?1:0}],gaps:{useLocationEmpty:0,storageLocationEmpty:0}});
    if(path==='/api/chemicals')return respond(200,{ok:true,products:[{id:'33333333-3333-4333-8333-333333333333',productName:'WD-40',manufacturer:'제조사',departmentNames:'시설팀',currentVersionId:uploaded?'55555555-5555-4555-8555-555555555555':null,reviewStatus:uploaded?'UNREVIEWED':null,updatedAt:'2026-09-21T00:00:00.000Z'}]});
    if(path==='/api/chemicals/33333333-3333-4333-8333-333333333333/versions'&&req.method==='POST'){uploaded=true;return respond(201,{ok:true,version:{id:'55555555-5555-4555-8555-555555555555',versionNo:1,isCurrent:true}});}
    ...
    if(path==='/api/chemicals/33333333-3333-4333-8333-333333333333')return respond(200,{ok:true,product:{id:'33333333-3333-4333-8333-333333333333',...productInfo,createdByName:'사용자',createdAt:'2026-09-21T00:00:00.000Z',currentVersionId:uploaded?'55555555-5555-4555-8555-555555555555':null,versionNo:uploaded?1:null,originalFilename:uploaded?'WD-40_MSDS.pdf':null,fileSize:uploaded?1048576:null,issueDate:uploaded?'2026-08-10':null,revisionDate:uploaded?'2026-08-14':null,submissionNumber:uploaded?'AA123':null,reviewStatus:uploaded?'UNREVIEWED':null,uploadedByName:uploaded?'관리자':null,uploadedAt:uploaded?'2026-09-21T00:00:00.000Z':null},usages:[{id:'44444444-4444-4444-8444-444444444444',departmentId:'22222222-2222-4222-8222-222222222222',departmentName:'시설팀',purpose:'윤활',useLocation:'기계실',storageLocation:'시설 자재창고',stockQuantity:5,stockUnit:'EA',averageUsageQuantity:3,averageUsagePeriod:'월',usageUnit:'EA',reportedByName:'사용자'}],versions:uploaded?[{id:'55555555-5555-4555-8555-555555555555',versionNo:1,originalFilename:'WD-40_MSDS.pdf',fileSize:1048576,revisionDate:'2026-08-14',isCurrent:1,reviewStatus:'UNREVIEWED',uploadedAt:'2026-09-21T00:00:00.000Z'}]:[]});
    if(path.startsWith('/api/'))return respond(401,{ok:false,error:'UNAUTHENTICATED'});
    if(path==='/pdf-stub.js')return res.writeHead(200,{'Content-Type':'text/javascript'}).end(`export const GlobalWorkerOptions={};export const OPS={};export function getDocument({data}){const source=new TextDecoder().decode(data);if(source.includes('FAIL'))return{promise:Promise.reject(new Error('parse failed'))};const lines=source.includes('PARTIAL')?['1. 화학제품과 회사에 관한 정보','제품명 | Partial Product','2. 유해성·위험성','3. 구성성분의 명칭 및 함유량']:['1. 화학제품과 회사에 관한 정보','제품명 | WD-40 Multi-Use Product','제조사 | WD-40 Company','공급자 | Korea Supplier','제품 코드 | WD40-001','작성일 | 2026-08-10','개정일 | 2026-08-14','제출번호 | AA12345','2. 유해성·위험성','신호어 | 위험','3. 구성성분의 명칭 및 함유량'];const items=lines.map((str,index)=>({str,transform:[1,0,0,10,0,1000-index*30],width:str.length*8,height:10}));return{promise:Promise.resolve({numPages:1,getPage:async()=>({getTextContent:async()=>({items}),cleanup(){}})})}}`);

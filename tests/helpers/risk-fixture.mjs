import { seedEmailProof } from './email-proof.mjs';
...
export async function fixture(t){const db=createTestDB();t.after(()=>db.close());for(const migration of ['0002_saved_documents.sql','0003_risk_assessment.sql'])db.sqlite.exec(readFileSync(new URL('../../migrations/'+migration,import.meta.url),'utf8'));const accounts=[];for(const email of ['risk-one@example.com','risk-two@example.com']){const input={email,password:'fixture password1',name:'테스터',companyName:'회사',departmentName:'부서',position:'직급'};const request=()=>new Request(origin+'/api/auth/test',{method:'POST',headers:{Origin:origin,'Content-Type':'application/json'},body:JSON.stringify(input)});input.emailVerificationProof=await seedEmailProof(db,input.email);assert.equal((await signup({request:request(),env:{DB:db}})).status,201);const response=await login({request:request(),env:{DB:db}});const user=(await response.json()).user;accounts.push({id:user.id,cookie:response.headers.get('set-cookie').split(';')[0]});}return{db,a:accounts[0],b:accounts[1]};}
